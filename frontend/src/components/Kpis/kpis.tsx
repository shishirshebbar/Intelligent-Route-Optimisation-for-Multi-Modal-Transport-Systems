...
const KpiCard = ({ title, value, sub }: KpiProps) => (
  <div
    className="
  ...
  >
    ...
    {sub && <div className="mt-1 text-[11px] text-slate-400/80">{sub}</div>}
  </div>
)
...
export default function Kpis(props: {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 md:grid-cols-9">
      ...
      {props.delayReductionPct !== undefined && (
        <KpiCard title="Delay Reduced" value={`${props.delayReductionPct}%`} sub="vs baseline" />
      )}
      {props.emissionsSavedPct !== undefined && (
        <KpiCard title="Emissions Saved" value={`${props.emissionsSavedPct}%`} sub="CO2 reduction" />
      )}
      {props.costChangePct !== undefined && (
        <KpiCard title="Cost Change" value={`${props.costChangePct}%`} sub="optimised vs baseline" />
      )}
      {props.reroutesCount !== undefined && (
        <KpiCard title="Reroutes" value={props.reroutesCount} sub="event-triggered" />
      )}
    </div>
  )
}
